...
int main(void) {
  ...
  for(i=0; i<n; i++){ // n repetições para os n alunos
	...
	// Calculo da Média da N1 dos Alunos...
    for(i=0; i<n; i++){
	n1 = listaAluno[i]->elementos[0]+listaAluno[i]->elementos[1]+listaAluno[i]->elementos[2];
	medianotasn1 = n1/3;
	printf ("\n A MÉDIA DA NOTA DO ALUNO %d É:  %.1f\t",i, medianotasn1);	
	n1 = 0;
	medianotasn1 = 0;
	}
	cont=0;
	...
	if (cont>0){
		printf ("\n\n A QUANTIDADE DE ALUNOS QUE TIRARAM NOTA 10 NA N1 É: %d \n\n", cont);	
	}else{
		printf ("\n\n NENHUM ALUNO TIROU NOTA 10 NA N1 \n\n,");
	}
system ("pause");
  
// Qual a média das notas dos alunos na N1?				
